...
class TestSingleRun:
    ...
    def test_calls_stepwise_pileup_generator_with_curr_index_file_and_bam(
            self, mocker, config):
        pileup_run_stub = PileupRunStubWithVisitorsAndCounters(config)
        generator_mock = mocker.patch(
            'mqc.mcall_run.stepwise_pileup_generator',
            return_value=[1, 2])
        index_file_mock = Mock(spec=IndexFile)
        counters = pileup_run_stub._single_run_over_index_file(
            index_file=index_file_mock)
        generator_mock.assert_called_with(index_file=index_file_mock,
    ...
    def test_for_all_motif_pileups_calls_process_method_on_all_visitors(
        ...
        counters = pileup_run_stub._single_run_over_index_file(
            index_file=Mock(spec=IndexFile))

        expected_calls = [call.process(1), call.process(2)]
